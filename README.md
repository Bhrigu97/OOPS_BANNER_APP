...
## UC4: Store Banner Lines in an Array

UC3 still required constructing each line and printing it immediately,
which led to repetitive code. In UC4 we populate a `String[]` with all
seven banner lines and then loop through the array to print them. This
change highlights array usage and greatly simplifies the print logic.

- Declare `String[] lines = new String[7];` and fill it in a simple loop.
- Use an enhanced for-loop (`for (String line : lines)`) to print each
  banner line.
- Keeps visual output identical to UC3 but greatly improves maintainability.

The compile/run instructions stay the same; behaviour is unchanged.
